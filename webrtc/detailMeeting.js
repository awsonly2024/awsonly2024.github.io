				...
				janus.attach(
					...
					//roomList();
						...
						onmessage: function(msg, jsep) {
							...
							if(event) {
								if(event === "joined") { //입장하는 사람(내가)이 기존 사람들을 구독
									myid = msg["id"];
								...
								} else if(event === "event") { //기존 사람들이 입장하는 사람(나를)을 구독
						...
						onlocalstream: function(stream) { //WebRTC 연결이 생성된 후 사용자의 카메라/마이크 스트림이 성공적으로 로드되었을 때 호출
							...
							if(username === "mentor"){
								//$('#videolocal').insertAfter('#mentor')
								mentorDiv.append($('#videolocal'));
							}
							...
							$('#videos').removeClass('hide').show();
							if($('#myvideo').length === 0) {
								$('#videolocal').append('<h3>내 이름 : &nbsp;&nbsp;&nbsp;<span class="label label-primary hide" id="publisher"></span></h3>')
								...
								$('#videolocal').append('<button id="unpublish">Unpublish</button>');
						...
						oncleanup: function() {
							...
							//$('#mentor .videoremote').remove();
							//$('#mentor').empty();
							//$('#mentor').html('<button id="publish">Publish</button>');
						}
					});
			},
			error: function(error) {
	...
	janus.attach(
		{
			...
			onremotestream: function(stream) {
				...
				if($('#remotevideo'+remoteFeed.rfindex).length === 0) {
					...
					$('#videoremote'+remoteFeed.rfindex).append('<div><h3>Remote Video #' + remoteFeed.rfindex + '<span class="label label-info hide" id="remote1"style="color:red">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + remoteFeed.rfdisplay + '</span></h3></div>');
					$('#videoremote'+remoteFeed.rfindex).append('<video class="rounded centered" id="waitingvideo' + remoteFeed.rfindex + '" width="100%" height='+ heightSet +' />');
					$('#videoremote'+remoteFeed.rfindex).append('<video class="rounded centered relative hide" id="remotevideo' + remoteFeed.rfindex + '" width="100%" height='+ heightSet +' autoplay playsinline/>');
